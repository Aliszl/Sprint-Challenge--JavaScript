...
console.log(dinosaur1, dinosaur2, dinosaur3);
...
console.log(`${dinosaur2.name} was ${dinosaur2.bodyLengthM} m long`);
...
let universities = [];
graduates.forEach((currentItem) => {
  universities.push(currentItem.university)
});
...
const contactInfo = [];
graduates.forEach((currentItem) => {
  contactInfo.push(`${currentItem.first_name } ${currentItem.email}`);
});
...
let unisWithUni = graduates.filter(function (currentItem) {
  return currentItem.university.includes("Uni");
});
...
let lowPopulationAnimals = zooAnimals.filter((currentItem) => {
